...
st.write("""This module is available only for ensemble mode and is designed to 
validate the predictions of AlloSigma2 using a Protein Structure Network (PSN)-based method.

The module relies on the output of the Long Range module, specifically the mutations predicted 
to affect pocket residues. These predictions are then used as input for identifying potential 
communication paths between mutations and their respective pocket sites using PyInteraph2. 
We retain only paths that are 4 residues in length or more. If such a path is identified, 
it is considered a validation of the AlloSigma2 predicted allosteric effect of a mutation
on the pocket residue(s).

The module aims to leverage these two methods to validate the predictions and use a 
consensus approach to build the final classification of a mutation's long-range effects.
...
data = [ ( 'damaging'       , 'The mutation effect on pocket site(s) was identified by both methods'),
         ...
         ( 'uncertain'      , 'The the two methods are not in agreement, or the mutation could not be assessed by AlloSigma2') ]
st.table(pd.DataFrame(data, columns=['Value', 'Meaning']))
