...
on adding more in adding more in following releases''')
...
mode = st.selectbox("Select MAVISp mode", options=['simple mode', 'ensemble mode'], index=None)

if mode is None:
    st.stop()

if mode == 'simple mode':
    current_mode = 'simple_mode' 
else:
    current_mode = 'ensemble_mode'

try:
    main_table = load_main_table(database_dir, 'current_mode')
except FileNotFoundError:
    st.write(f"No entries are currently available for {current_mode}.")
    st.stop()

gene = st.selectbox("Select gene", options=main_table['Protein'].to_list(), index=None)

st.stop()

try:
    sm_table = load_main_table(database_dir, 'simple_mode')
except FileNotFoundError:
    st.write('No entries are currently available for simple mode.')
    st.stop()

try:
    em_table = load_main_table(database_dir, 'ensemble_mode')
except FileNotFoundError:
    st.write('No entries are currently available for simple mode.')
    st.stop()
