...
Further, ThermoMPNN was used to predict changes of folding free energy in simple and ensemble mode
including a stability classification according to a ThermoMPNN-specific classification scheme (see below). 
For ThermoMPNN, see H. Dieckhaus,M. Brocidiacono,N.Z. Randolph, & B. Kuhlman,  
...
data = [ ("Stability (FoldX5, alphafold, kcal/mol)",
         ...
         "value (kcal/mol)"),
         ("Stability (ThermoMPNN, alphafold, kcal/mol)",
         "change of folding free energy upon mutation calculated using ThermoMPNN",
         "value (kcal/mol)"),
...
The final classification is derived from these values. 

We support different types of classification, depending on the involved predictors:

**Consensus between Rosetta or RaSP and FoldX**

For this classification, the classification column is always generated by considering two methods at the time,
either FoldX and Rosetta, or FoldX and RaSP. This means that the classification will only be available if
the respective datasets are also available.
...
not agree, the final classificaiton will be Uncertain. 

**Foldetta**

For Foldetta and the mean of FoldX and RaSP, no consensus method is used, the mean change of folding free energy
is classified according to the table in the previous section.

**ThermoMPNN**

For ThermoMPPN, classification is performed as follows:""")

data = [ ( 'DDG >= 2',                       'Destabilizing'),
         ( 'DDG <= -2',                      'Stabilizing'),
         ( '-1 < DDG < 1',                   'Neutral'),
...
st.write("""This module annotates whether mutations are predicted to have an effect
on disulfide bridges in the protein. It evaluates the impact of the variants that
alter a wild-type cysteine or introduce a cysteine upon mutation, on the disruption
and formation of disulfide bridges.

To check the effect of mutations on native disulfide bridges, we first identify existing
disulfide bridges from the protein structure, by finding pairs of cysteines that satisfy
a set of geometrical criteria. Variants that substitute any of the cysteines partecipating
in the identified native disulfide bonds are classified as damaging; all other mutations
are classified as neutral.

To check if the substitution to Cysteine leads to the formation of new disulfides, we
investigate whether the variant can induce new disulfide bonds in structures of mutants
that are generated by FoldX. We check for the presence of disulfide bridges in these
mutated structures by means of geometrical criteria. Variants that cause a new disulfide
bridge to be identified are classified as damaging; all other others as neutral.

The module produces the following columns:""")

data = [ ( "Loss of disulfide bridge",
           "Whether the mutation is predicted to cause the loss of a disulfide bridge (damaging) or not (neutral)",
           "`neutral` or `damaging`"),
         ( "Predicted de-novo disulfide bridge",
           "Whether the mutation is predicted to cause the formation of a novel disulfide bridge (damaging) or not (neutral)",
           "`neutral` or `damaging`") ]
st.dataframe(pd.DataFrame(data, columns=['Column', 'Description', 'Possible values']))
